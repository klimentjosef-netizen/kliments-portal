-- ===========================================================================
-- 0007 · Živý přehled účetnictví klienta (předběžně z dokladů)
--
-- kl_prehled(klient, rok) vrací JSON pro úvodní obrazovku portálu:
--   aktualnost · banka · saldo · dph · dan · chybi
-- Počítá z vytěžených dokladů a banky, bez účetních úprav a odpisů, proto
-- "předběžně". Běží s právy volajícího (RLS klienta platí).
-- ===========================================================================

-- Částka dokladu v CZK: přepočtená, nebo rovnou částka u CZK dokladu
CREATE OR REPLACE FUNCTION public.kl_czk(d public.documents)
RETURNS numeric LANGUAGE sql IMMUTABLE AS $$
  SELECT coalesce(d.amount_czk, CASE WHEN d.currency = 'CZK' THEN d.amount_total END)
$$;

-- DPH dokladu v CZK (poměrem přepočtu celé částky)
CREATE OR REPLACE FUNCTION public.kl_dph_czk(d public.documents)
RETURNS numeric LANGUAGE sql IMMUTABLE AS $$
  SELECT CASE
    WHEN d.amount_vat IS NULL THEN 0
    WHEN d.currency = 'CZK' THEN d.amount_vat
    WHEN d.amount_czk IS NOT NULL AND d.amount_total <> 0 THEN round(d.amount_vat * d.amount_czk / d.amount_total, 2)
    ELSE 0 END
$$;
...
BEGIN
  ...
  SELECT
    ...
    coalesce(sum(kl_czk(d) - CASE WHEN k.vat_payer THEN kl_dph_czk(d) ELSE 0 END) FILTER (WHERE kind IN ('received_invoice', 'receipt')), 0)
...
GRANT EXECUTE ON FUNCTION public.kl_prehled(uuid, int) TO authenticated;

-- Klient smí číst soubory svých dokladů (klienti/{client_id}/...)
CREATE POLICY docs_client_files ON storage.objects FOR SELECT
  USING (
    bucket_id = 'documents'
    AND (storage.foldername(name))[1] = 'klienti'
    AND public.kl_can_read(((storage.foldername(name))[2])::uuid)
  );
