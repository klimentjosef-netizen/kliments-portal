...
// pro příjemce;Poznámka;Typ) a Raiffeisenbank "Pohyby" (Datum zaúčtování, VS, Id transakce,
// Původní částka/měna, Název obchodníka).
import './lib/env.mjs'
...
function kategorie(typ, zprava) {
  if (/karetn|kartou|na internetu/i.test(typ ?? '')) return 'card'
...
export async function importujCsv({ ico, ucet, soubor, nasucho = false }) {
  ...
  const { data: acc, error: ae } = await db.from('bank_accounts')
    .upsert({ client_id: klient.id, number: ucet, currency: 'CZK', name: ucet.endsWith('/2010') ? 'Fio banka' : ucet.endsWith('/5500') ? 'Raiffeisenbank' : null }, { onConflict: 'client_id,number' })
    .select('id').single()
  if (ae) throw ae

  const [hlavicka, ...radky] = csv(fs.readFileSync(soubor, 'utf8').replace(/^﻿/, ''))
  // sloupec podle prvního nalezeného názvu (Fio / Raiffeisenbank)
  const i = (...nazvy) => {
    for (const n of nazvy) { const x = hlavicka.findIndex((h) => h.trim().toLowerCase() === n); if (x >= 0) return x }
    return -1
  }
  const col = {
    datum: i('datum zaúčtování', 'datum', 'datum provedení'), objem: i('objem', 'zaúčtovaná částka'),
    mena: i('měna', 'měna účtu'), protiucet: i('protiúčet', 'číslo protiúčtu'), kod: i('kód banky'),
    zprava: i('zpráva pro příjemce', 'zpráva'), poznamka: i('poznámka'), typ: i('typ', 'typ transakce'),
    vs: i('vs'), ks: i('ks'), ss: i('ss'), id: i('id pohybu', 'id transakce'),
    nazev: i('název protiúčtu'), obchodnik: i('název obchodníka'),
    puvodni: i('původní částka'), puvodniMena: i('původní měna'),
  }
  ...
  const tx = radky.map((r) => {
    const g = (k) => {
      const v = col[k] >= 0 ? prazdne(r[col[k]]) : null
      return k === 'datum' && v ? v.slice(0, 10) : v // "22.09.2026 09:45" -> datum
    }
    ...
    return {
      ...
      dedup_key: g('id') ? `id:${g('id')}` : `csv:${otisk}:${poradi.get(otisk)}`,
